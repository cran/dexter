...
#' @param parms An object returned by function \code{fit_enorm} containing parameter estimates. If parms are provided, item parameters are condidered known. 
...
#' @param use_draw When the ENORM was fitted with a Gibbs sampler, this specifies the use of a particular sample of item parameters used to generate the plausibe value(s). If NULL, the posterior means are used. If outside range, the last iteration will be used.
                             ...
                             merge_within_persons=merge_within_persons)
{
  ...
  from = Gibbs.settings$from.pv ; step = Gibbs.settings$step.pv # burnin and thinning for pvs
  ...
  } else
  {
    ...
    respData = get_resp_data(respData, summarised=TRUE, extra_columns=covariates, 
                             protect_x=!inherits(dataSrc,'DBIConnection'))
  } 
  ...
  if(any(is.na(design$first))) stop('Some of your items have no parameters')
